...
fx_ver_t fx_muxer_t::resolve_most_compatible(const std::vector<fx_ver_t>& version_list,
    ...
    const int& roll_fwd_on_no_candidate_fx)
{
    ...
    if (!specified.is_prerelease())
    {
        ...
        if (patch_roll_fwd)
        {
            ...
            for (const auto& ver : version_list)
            {
                trace::verbose(_X("Inspecting version... [%s]"), ver.as_str());
    ...
    const int& roll_fwd_on_no_candidate_fx_val)
{
    ...
    for (pal::string_t dir : hive_dir)
    {
        ...
        fx_ver_t resolved_ver = specified;
        if (!do_roll_forward)
        {
        ...
        else
        {
            ...
            resolved_ver = resolve_most_compatible(version_list, fx_ver, specified, patch_roll_fwd_enabled, roll_fwd_on_no_candidate_fx_val);
        }
